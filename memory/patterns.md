...
[2026-08-13] Pattern: read localStorage through `useSyncExternalStore`, never an effect — `react-hooks/set-state-in-effect` is an error here, and the server has no localStorage. getSnapshot MUST return a cached value (see lib/builder/storage.ts, which caches per key and invalidates on write) or React re-renders forever. Use a stable server snapshot so hydration matches, and a distinct `undefined` sentinel when "not read yet" has to be told apart from "not found".
