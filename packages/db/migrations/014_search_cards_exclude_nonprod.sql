-- Migration 014: exclude non-playable cards from search_cards
--
-- Filters out token sheets, art series, and memorabilia via the sets table.
-- The cards table has no set_type column (migration 007 was never applied to
-- production), so we use a NOT IN subquery on sets.set_type instead.
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  ...
  FROM (
    ...
    WHERE
      ...
      -- Exclude token sheets, art series, and memorabilia
...
GRANT EXECUTE ON FUNCTION search_cards(text, text, boolean, integer, text[], integer[], text[], text[], text[], text, boolean, uuid) TO anon, authenticated;
